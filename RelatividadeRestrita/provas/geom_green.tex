...
Justifique todas as respostas. Sempre que possível, interprete fisicamente os resultados.
      ...
      \item Equação da Onda e Função de Green
            ...
            \begin{enumerate}
                  \item Defina a função de Green $G(t, \vec{x}; t', \vec{x}')$ associada
                        ...
                        e explique sua interpretação física, destacando as condições de causalidade para as funções de Green retardada e avançada.

                  \item A função de Green retardada em 3 dimensões espaciais pode ser expressa como
                        $$
                              G_{\mathrm{ret}}(ct - ct', \vec{x} - \vec{x}') =
                              \frac{\delta\left(ct - ct' - |\vec{x} - \vec{x}'|\right)}{4\pi |\vec{x} - \vec{x}'|}.
                        $$
                        Explique o significado físico do suporte desta função estar limitado à superfície do cone de luz.
